...
from diffusion_policies.policy.diffusion_unet_timm_policy import DiffusionUnetTimmPolicy
...
class TrainDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        # print("train dataloader:", [data for data in dataset])

        # BUG: apply this normalizer to env_runner
        ...
        # NOTE: set normalizer to the model (policy)
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                # import ipdb; ipdb.set_trace() no bug
                train_losses = list()
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                        leave=False, mininterval=cfg.training.tqdm_interval_sec) as tepoch:
                    for batch_idx, batch in enumerate(tepoch):

                        # import ipdb; ipdb.set_trace() bug
                        ...
                        if (cfg.training.max_train_steps is not None) \
                            ...
                            cprint(f"Reach max training steps:{cfg.training.max_train_steps}", 'light_green')
                            break
                ...
                # checkpoint
                if (self.epoch % checkpoint_every) == (checkpoint_every - 1) and accelerator.is_main_process:
